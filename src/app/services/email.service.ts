...
export class MailService {
  ...
  async sendVerification(userDetails: UserDetails) {
    const subject = 'Welcome to Tournest';
    const template = 'welcome-to-tournest';
  ...
  async sendForgotPassword(userDetails: UserDetails, token: number) {
    ...
    const subject = 'Tournest - Your Password Reset Token.';
  ...
  private generateMailData = (to: string, subject: string, template: string, variables: MailVariables) => {
    return {
      to,
      from: `Tournest <${this.configService.get(APP_CONFIG.MAILGUN_FROM)}>`,
      subject,
      template,
